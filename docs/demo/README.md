...
## 七、在 Claude Code 客户端里运行（使用体验）

本 skill 作为 Claude Code skill 的实际运行界面：左侧文件区是榨好的笔记，右侧助手按 `SKILL.md` 流程核对章节、整理摘要。截图客户端为 muselab（自托管 AI workspace）。

> 本张来源与前 6 张不同：张小珺 · 商业访谈录 #140《对姚顺宇的 4 小时访谈》（单集时长 03:50:35）。版权归原播客所有，截图仅演示运行界面，已隐去本机用户名与本地路径。

![在 Claude Code 客户端里运行](07-in-muselab.png)
